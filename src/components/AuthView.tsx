  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        throw new Error('API route not found (404). Please verify backend server route deployment.');
      }
      if (response.status === 500) {
        throw new Error('Server internal error (500). Please check server logs.');
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      try {
        ...
        const signupData = await parseJsonResponse(signupRes);
        if (!signupRes.ok) {
